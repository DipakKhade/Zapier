...
import { MetadataModal } from "@/components/metadata-model";
          ...
          {selectedNode && (
            <DialogContent>
              <DialogHeader>
                <DialogTitle>
                  Select{" "}
                  {selectedNode.data.label === "Trigger" ? "Trigger" : "Action"}
                </DialogTitle>
                <DialogDescription>
                  <div className="space-y-2">
                    <SelectionModal
                      modalFor={
                        selectedNode.id === "1" ? "Triggers" : "Actions"
                      }
                      data={
                        selectedNode.id === "1"
                          ? availableTriggers
...
const SelectionModal = ({
  ...
  const search = (searchArgs: string) => {
    setLocalData(
      backUpData.filter((x) =>
        x.name.toLowerCase().includes(searchArgs.trim().toLowerCase())
      )
    );
  };
    ...
    <div>
      <div className="flex">
        ...
        <input
          type="text"
          id="searchBox"
          placeholder={`Search ${modalFor}`}
          className="border border-slate-50 p-2 w-full m-2"
          onChange={(e) => {
      ...
      <div className="space-x-2.5 grid grid-cols-2 space-y-2 pt-4">
        {localData.map((ele, index) => {
          return (
            <div key={index}>
              <button
                className="cursor-pointer font-bold text-lg text-start flex items-center justify-center space-y-2 p-2"
                onClick={() => {
                  // setSelectedData([{...ele}])
